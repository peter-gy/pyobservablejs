import type { AnyWidget, Initialize, InitializeProps, RenderProps } from "@anywidget/types";
import { ensureLifecycleSignal, instantiateAnyWidgetBundleApp } from "./entry";
...
type InitializeResult<ModelState extends AnyWidgetState> = Awaited<ReturnType<Initialize<ModelState>>>;
...
export function createAnyWidgetBundleDevelopmentEntry<ModelState extends AnyWidgetState>(
	...
	const widget: AnyWidget<ModelState> = () => {
		...
		const initialize = async (props: InitializeProps<ModelState>) => {
			...
			const operation = runAfter(state.operation, async () => {
				result = await startModel(state, currentApp);
			});
	...
	return {
		...
		async update(app) {
			...
			// Anywidget retains object-valued initialization exports by reference.
			// Updating their producer requires the host to bind a new entry.
			if (activeModels.some((state) => state.exportMode === "object")) requireObjectExportRebind();
			currentApp = app;
			await settleOperations(
				activeModels.map((state) => {
					const operation = runAfter(state.operation, async () => {
						if (state.disposed) return;
						if (state.exportMode === "object") requireObjectExportRebind();
...
async function startModel<ModelState extends AnyWidgetState>(
...
): Promise<InitializeResult<ModelState>> {
	...
	state.initializeCleanup = typeof result === "function" ? (result as Cleanup) : undefined;
	updateModelExports(state, result);
	return result;
}
...
function updateModelExports<ModelState extends AnyWidgetState>(
	state: DevelopmentModel<ModelState>,
	result: InitializeResult<ModelState>,
): void {
	const mode = typeof result === "object" && result !== null ? "object" : "non-object";
	if (state.exportMode !== undefined && state.exportMode !== mode) {
		throw new Error("Anywidget bundle initialize exports changed shape during a hot update.");
	}
	state.exportMode = mode;
}

function requireObjectExportRebind(): never {
	throw new Error("Anywidget bundle object initialization exports require a hot rebind.");
}

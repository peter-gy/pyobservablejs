...
import { createAnyWidgetBundleDevelopmentEntry } from "@/anywidget-bundle/dev";
...
describe("anywidget bundle development entry", () => {
	test("instantiates factories per model and preserves lifecycle props and exports", async () => {
		const initialize = vi.fn((props: InitializeProps<WidgetModel>) => ({ model: props.model }));
		const render = vi.fn((props: RenderProps<WidgetModel>) => {
			props.el.textContent = "ready";
		});
		const factory = vi.fn(async () => ({ initialize, render }));
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>(factory);
		...
		await expect(first.initialize?.(firstInitialization)).resolves.toEqual({ model: firstModel });
		await expect(second.initialize?.(secondInitialization)).resolves.toEqual({
			model: secondModel,
		});
	...
	test("owns lifecycle signals when a host omits them", async () => {
		...
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({
			initialize(props) {
	...
	test("restarts active models and views when the app updates", async () => {
		...
		const firstRender = vi.fn(({ el }: RenderProps<WidgetModel>) => {
			el.textContent = "first";
			return firstRenderCleanup;
		});
		const secondRender = vi.fn(({ el }: RenderProps<WidgetModel>) => {
	...
	test("returns object-valued exports unchanged and requires a hot rebind", async () => {
		class ModelExports {
			#status = "ready";
		...
		const initialExports = new ModelExports();
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({
			initialize: () => initialExports,
		});
		const definition = await widgetDefinition(entry.widget);
		const exports = await definition.initialize?.(initializeProps(createModel({}), new AbortController().signal));

		expect(exports).toBe(initialExports);
		expect(initialExports.status()).toBe("ready");
		await expect(entry.update({ initialize: () => new ModelExports() })).rejects.toThrow(
			"object initialization exports require a hot rebind",
		);
		await entry.dispose();
	});

	test("requires a hot rebind when object exports settle during an update", async () => {
		let releaseInitialize!: () => void;
		const exports = { version: "first" };
		...
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({ initialize });
		const definition = await widgetDefinition(entry.widget);
		const initialization = definition.initialize?.(initializeProps(createModel({}), new AbortController().signal));
		await vi.waitFor(() => expect(initialize).toHaveBeenCalledOnce());

		const update = entry.update({ initialize: () => ({ version: "second" }) });
		const rejection = expect(update).rejects.toThrow("object initialization exports require a hot rebind");
		releaseInitialize();

		await expect(initialization).resolves.toBe(exports);
		await rejection;
		await entry.dispose();
	});

	test("requires a rebind when initialization exports change shape", async () => {
		const cleanup = vi.fn();
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({
			initialize: () => cleanup,
		});
		const definition = await widgetDefinition(entry.widget);
		await definition.initialize?.(initializeProps(createModel({}), new AbortController().signal));

		await expect(entry.update({ initialize: () => ({ ready: true }) })).rejects.toThrow(
			"initialize exports changed shape",
		);
		await entry.dispose();
	...
	test("waits for delayed initialization before final listener cleanup", async () => {
		...
		const cleanup = vi.fn();
		const initialize = vi.fn(async ({ model }: InitializeProps<WidgetModel>) => {
			await new Promise<void>((resolve) => {
				releaseInitialize = resolve;
			});
	...
	test("waits for delayed render before final listener cleanup", async () => {
		...
		const listener = vi.fn();
		const cleanup = vi.fn();
		const render = vi.fn(async ({ model }: RenderProps<WidgetModel>) => {
			await new Promise<void>((resolve) => {
		...
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({ render });
		const definition = await widgetDefinition(entry.widget);
		const model = createModel({});
	...
	test("settles cleanup failures after an abort", async () => {
		...
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({ initialize: () => cleanup });
		const definition = await widgetDefinition(entry.widget);
		const controller = new AbortController();
		await definition.initialize?.(initializeProps(createModel({}), controller.signal));
	...
	test("finishes view cleanup before initialization cleanup starts", async () => {
		...
		const events: string[] = [];
		const initializeCleanup = vi.fn(() => events.push("initialize"));
		const renderCleanup = vi.fn(
			() =>
				new Promise<void>((resolve) => {
	...
	test("waits for every rerender before surfacing a view failure", async () => {
		...
		const nextRender = vi.fn(({ el }: RenderProps<WidgetModel>) => {
			if (el.dataset.result === "failure") throw new Error("rerender failed");
			return new Promise<() => void>((resolve) => {
				releaseRender = () => resolve(lateCleanup);
			});
		});
		const entry = createAnyWidgetBundleDevelopmentEntry<WidgetModel>({ render: vi.fn() });
		const definition = await widgetDefinition(entry.widget);
		const model = createModel({});
		await definition.initialize?.(initializeProps(model, new AbortController().signal));

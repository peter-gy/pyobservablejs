---
title: Filter in Observable
description: Use a Notebook Kit input to filter a cars scatter plot.
---

# Filter in Observable

Notebook Kit inputs handle interaction in the browser. Choose a cylinder count
to update the scatter plot. `cars` is a sample dataset provided by Notebook
Kit.

```marimo-config
requires-python = ">=3.11"
dependencies = ["pyobservablejs"]
```

```python
import marimo as mo
import observablejs as obs

notebook = obs.Notebook(
    obs.js(
        """
        const cylinders = view(Inputs.select(
          ["All", ...new Set(cars.map((d) => d.cylinders))],
          {label: "Cylinders", value: "All"}
        ));
        """,
        key="cylinder_control",
    ),
    obs.js(
        """
        const filteredCars = cylinders === "All"
          ? cars
          : cars.filter((d) => d.cylinders === cylinders);
        """,
        key="filtered_cars",
        display=False,
    ),
    obs.js(
        """
        Plot.dot(filteredCars, {
          x: "weight (lb)",
          y: "economy (mpg)",
          fill: "cylinders",
          r: 4,
          opacity: 0.72,
          tip: {fill: "var(--marimo-island-background)"}
        }).plot({
          height: 320,
          color: {legend: true},
          x: {grid: true, label: "Weight (lb)"},
          y: {grid: true, label: "Fuel economy (mpg)"}
        })
        """,
        key="chart",
    ),
)

full_view = notebook.view()
mo.ui.anywidget(full_view)
```

```python marimo echo=false server-output=false
import marimo as mo
import observablejs as obs

notebook = obs.Notebook(
    obs.js(
        """
        const cylinders = view(Inputs.select(
          ["All", ...new Set(cars.map((d) => d.cylinders))],
          {label: "Cylinders", value: "All"}
        ));
        """,
        key="cylinder_control",
    ),
    obs.js(
        """
        const filteredCars = cylinders === "All"
          ? cars
          : cars.filter((d) => d.cylinders === cylinders);
        """,
        key="filtered_cars",
        display=False,
    ),
    obs.js(
        """
        Plot.dot(filteredCars, {
          x: "weight (lb)",
          y: "economy (mpg)",
          fill: "cylinders",
          r: 4,
          opacity: 0.72,
          tip: {fill: "var(--marimo-island-background)"}
        }).plot({
          height: 320,
          color: {legend: true},
          x: {grid: true, label: "Weight (lb)"},
          y: {grid: true, label: "Fuel economy (mpg)"}
        })
        """,
        key="chart",
    ),
)

full_view = notebook.view()
mo.ui.anywidget(full_view)
```

The input defines `cylinders`. The hidden cell derives `filteredCars`. The chart
references that derived value, so the graph contains both dependency edges.

## Continue

- [Observable cells and reactivity](../guides/author-cells.mdx) explains
  expression cells, program cells, and graph invalidation.
- [Values back to Python](../guides/cell-values.mdx) reads synchronized values
  after render.

...
- **Choose where each name is set.** Python sets `floor`. The browser sets
  ...
  Observable](../guide/connect/shared-inputs.mdx) when
...
See [Send Python values](../guide/connect/python-values.mdx) and [Read browser
results](../guide/connect/browser-results.mdx) for each direction in isolation.

...
same_view = notebook.view(notebook.cells["readout"])
```

## Python defines, the browser runs

Python sends the notebook and its values to the browser. [Notebook
Kit](https://observablehq.com/notebook-kit/), Observable's notebook format and
execution library, runs the cells there. [anywidget](https://anywidget.dev/)
connects each rendered view to Python. Each view sends structured results back
unless created with `capture_state=False`.
...
theme. [Traitlets](https://traitlets.readthedocs.io/) provides Python callbacks
for changes to these attributes:

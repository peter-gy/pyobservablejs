...
Use `await view.ready()` in Jupyter to raise current browser failures as Python
exceptions. In a reactive marimo cell, read `view.value` and call
`view.raise_for_errors()`. The exception includes structured diagnostics, cell
source, and browser stacks. See [Errors and diagnostics](../reference/errors.mdx).
...
the Python variable. See the naming rules in [Variables and
...
The cell must define a named output, such as `const total = rows.length;`.
Anonymous HTML and chart expressions render output but have an empty `values`
mapping. The defining cell must also be selected by the view. Hidden
dependencies evaluate, but their values are excluded from that view's results.
...
view's `value` in a separate reading cell. This subscription is specific to
marimo:
...
```python
full_view.value  # marimo subscription
full_view.state
```

In Jupyter, rerun the reading cell or attach a `state` observer as described in
[Read browser results](connect/browser-results.mdx#react-to-state-changes).
...
The constructor fetches original notebook source on every call: network failures

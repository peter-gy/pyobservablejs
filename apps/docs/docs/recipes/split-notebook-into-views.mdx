...
- **Each view still owns its readback.** `chart_view.runtime_values` and
  `summary_view.runtime_values` come from different runtimes and synchronize
  independently.
...
## When to use a composite view instead
...
in a single runtime. Separate views are for separate placements — different
cells of the host notebook, different layout columns, or different pages of
a dashboard. See [Select cells and create

...
recipe passes per-species statistics from the [Palmer Penguins
...
- A pandas or Polars `DataFrame` works in place of the literal list; it
  ...
  serialization](../reference/variables-and-serialization.mdx#python-to-browser).
...
- Update the records after render with
  ...
  Observable](../connect/python-to-observable.mdx).
- For large tables, register a file with
  [`files=`](../create/files-and-modules.mdx) and load it in the browser
  with `FileAttachment(...).csv()` instead of shipping records through
  widget state.

...
description: Typed input mappings, selector aliases, and immutable state records.
---
...
`NotebookRead` carries an explicit result. These records are available under
`obs.types`. Custom exceptions and diagnostic records live under
[`obs.errors`](errors.mdx).

See [Data access](data-access.mdx) for selectors, formats, and lifecycle.

...
| `mode`              | `js`, `ts`, `ojs`, `md`, `html`, `tex`, `dot`, `sql`, `sql.view`, `node`, `python`, or `r`.       |
...
`node`, `python`, and `r` preserve Notebook Kit interpreter cells. Their browser
evaluation reads precomputed files from Notebook Kit's `.observable/cache`
directory. Serve those files with the host page when importing such a document.
To calculate with the host Python kernel, pass results through
[`variables`](../guide/connect/python-values.mdx).

For SQL cells, `notebookkit_attrs={"database": "var:db"}` selects a database
defined as the JavaScript variable `db`. `mode="sql"` evaluates the query and
publishes its full result under `output`. `mode="sql.view"` publishes a reusable
query under `output`. Call that query's `.query()` method from another cell to
execute it against the bound database.

...
`view()` to create a renderable view of all cells or a selected group of cells.
...
Creates a Python-authored notebook definition and session. See [Cells](cells.mdx),
[variables](variables.mdx), [file attachments](file-attachments.mdx), and
[notebook themes](notebook-themes.mdx) for the nested input contracts.
...
Returns a new `NotebookView`. With `cells=None`, the view selects all cells in
notebook order. `cells` accepts a sequence containing cell indices, Python cell
...
created from the same notebook share Python variables and [supported `viewof`
input values](variables.mdx#python-values-and-viewof-inputs).

See [Views and composition](../guides/views-and-composition.mdx) for the full,
...
composite selection. Its browser runtime starts when an anywidget host mounts
it.
...
See [Values and graph](values-and-graph.mdx) for decoded value types and graph
records.
...
[File attachments](file-attachments.mdx) for path resolution and serialized HTML
boundaries.
...
[Variables](variables.mdx) for serialization and mutation contracts.
...
described by [File attachments](file-attachments.mdx) and
[Variables](variables.mdx). ObservableHQ-derived HTML preserves classic
...
[Source notebooks](source-notebooks.mdx) for accepted source shapes, defaults,

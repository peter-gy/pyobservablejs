...
render_only_view = notebook.view(capture_state=False)
```
...
`capture_state` defaults to `True`. Set it to `False` to render while leaving
the Python `ViewState` at its initial snapshot. Rendering, Python variable
updates, and shared browser inputs remain active.

...
| `values`             | `dict[str, Any]`         | Decoded values with one owning cell in this view.              |
...
`values` and `cell_values` require a completed view render. `graph` requires
a graph snapshot. Indexing `values` raises `KeyError` when the rendered value
mapping has no matching name. A name produced by multiple selected cells is
omitted from `values` because it has no unique owner.
...
| `key`    | `Optional[str]`  | Python selection key, or `None` when the cell has no key. |
...
| --------------------------------------- | --------------------- |
| `NotebookView.values` and `cell_values` | Completed view render |
...
Returns the `CellInfo` whose `index` equals `index`. It raises `IndexError`
when the graph has no matching cell.
...
| `key`             | `Optional[str]`   | Python selection key.                            |

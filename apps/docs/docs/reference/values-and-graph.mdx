...
`values` uses JavaScript output names, such as `total` from
`const total = rows.length;`. Keyed expression cells capture their displayed
value under the cell key. Rendered elements become summaries. Results cover
selected cells. Dependencies outside the selection appear in the graph but have
no `CellResult` in the view.
...
A named output such as `const problem = new Error("invalid value");` has
succeeded. `values["problem"]` becomes `BrowserErrorValue(name, message)`.
Throwing that error rejects evaluation and produces `CellError` and an error
result.
...
`graph.to_mermaid()` returns a [Mermaid](https://mermaid.js.org/) `flowchart LR`
string. `graph.to_d2()` returns a [D2](https://d2lang.com/) diagram with
rightward layout. Both formats describe diagrams as text. Diagram labels use
...
See [Inspect the dependency graph](../guide/connect/dependencies.mdx) for a live
workflow.

...
`.sqlite()` returns Observable's `SQLiteDatabaseClient` with `query`, `queryRow`,
and parameterized `sql` methods. `describeTables()` and `describeColumns({table})`
return schema records. `describe(table)` returns an HTML table with its displayed
rows in `.value`.

The database loads through [sql.js](https://sql.js.org/), which runs SQLite in
the browser. Before calling `.sqlite()`, set `globalThis.observablejsSqlite` to an object with

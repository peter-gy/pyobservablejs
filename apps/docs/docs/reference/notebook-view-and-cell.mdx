...
`**view_options`. `capture_state` defaults to `True`. Set it to `False` to
render while leaving the Python `ViewState` at its initial snapshot.
Cell and notebook validation errors come from the matching constructor.
Factories validate view options before notebook construction, local file

...
`NotebookView`, an [anywidget](https://anywidget.dev/) that any anywidget host
can display. Browser evaluation begins when the host mounts the view.
...
[`Notebook.view`](../reference/notebook.mdx#create-views) for selection
...
When a named `viewof` input changes, the notebook records its value if it has
a [supported shared
shape](../reference/variables.mdx#python-values-and-viewof-inputs). Sibling
views, both existing ones and views mounted later, then write that value to
their matching input. A successful round trip dispatches `input` and `change`
events and updates dependent cell values. Use the same input type for a shared
name across views: a mismatched target can coerce an incompatible value before
the round-trip check suppresses those events.

## Python `.view()` and JavaScript `view(...)`

The name `view` appears on both sides of the boundary. The Python methods
create renderable `NotebookView` widgets. JavaScript `view(input)` inside a
cell displays a browser input and exposes its current value to the graph.
...
Moving the input changes `threshold`, and Notebook Kit reruns the cells that
reference it. Python `notebook.view()` decides which of those cells evaluate
and render together.

## Mount views in a host

Each host has its own display idiom. marimo wraps widgets with
`mo.ui.anywidget`:
...
Jupyter and most other hosts display a `NotebookView` when it is the final
...
Keep a view mounted while Python updates variables, and create a distinct
...
Continue with [values back to Python](cell-values.mdx) for readback and
[Python variables](python-variables.mdx) for live session updates.

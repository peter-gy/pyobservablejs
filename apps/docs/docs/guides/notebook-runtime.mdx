...
[Views and composition](views-and-composition.mdx) for selection, mounting, and
...
valid `id` and `version`, as described in [ObservableHQ
imports](observablehq.mdx).
...
Use [Files and source notebooks](source-html.mdx) for `FileAttachment`, local
...
they change. Runtime-owned builtin names cannot be Python variables. Rendered
values and graph metadata synchronize back to the `NotebookView` that produced
them.

Continue with [Update from Python](python-variables.mdx) for the mounted-view
workflow and [Values back to Python](cell-values.mdx) for readback.

---
title: Display views
description: Mount NotebookView in anywidget hosts and manage its lifecycle.
---

# Display views

`notebook.view()` returns a `NotebookView`. Browser evaluation starts when an
anywidget host mounts it.
...
In marimo, wrap the view with `mo.ui.anywidget(view)`. In JupyterLab, Jupyter
Notebook, VS Code notebooks, and Google Colab, leave the view as the final cell
expression or pass it to `IPython.display.display`.
...
Continue with [Select cells](select-cells.mdx) or [Widget
composition](widget-composition.mdx).

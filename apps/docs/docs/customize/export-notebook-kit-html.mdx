...
The document is the _definition_, not the session. Two kinds of state stay
in Python:
...
Python cell `key` values are Python-side handles and never appear in
serialized HTML. Set the first-class `name=` cell option for identity that
must survive export.

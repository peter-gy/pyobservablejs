...
resolution. Use `from_observablehq_nodes` for node collections whose imports
should resolve from the specifiers stored in those nodes.
...
Every ObservableHQ constructor selects the *observable* runtime profile: the
classic Observable standard library instead of the Notebook Kit builtins.
This is why `require` works in imported notebooks but not in Python-authored
ones. The profile is fixed for the notebook session and survives

...
current working directory. `from_html_file` uses the HTML file's parent
directory. See [File attachments](../reference/file-attachments.mdx) for
path resolution, record shapes, and browser access methods.

## Discover files in source HTML

`Notebook.from_html` and `from_html_file` can discover local files referenced
by literal `FileAttachment("name")` calls and embed them as data URLs:
...
no filesystem location. `from_html_file` uses the document's parent
directory.

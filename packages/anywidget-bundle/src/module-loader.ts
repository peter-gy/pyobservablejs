...
import { isJavaScriptArtifactPath } from "./artifact-path.ts";
import { createBundleId } from "./id.ts";
import type { ModuleReader } from "./protocol.ts";
...
function revokeUrl(state: ModuleLoaderState, url: string): void {
	if (state.options.revokeModuleUrl) state.options.revokeModuleUrl(url);

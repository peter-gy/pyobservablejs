...
import anywidgetBundle, { type AnyWidgetBundleOptions } from "../src/vite.ts";
...
describe("anywidget bundle Vite plugin", () => {
	test("builds a configured artifact layout and manifest-backed module graph", async () => {
		...
		const entryUrl = `data:text/javascript;base64,${Buffer.from(entry).toString("base64")}#${Date.now()}`;
	...
	test("resolves a package app through Vite's module pipeline", async () => {
		const fixture = await createFixture({
			"node_modules/@fixture/app/package.json": JSON.stringify({
				name: "@fixture/app",
				type: "module",
				exports: "./index.js",
			}),
			"node_modules/@fixture/app/index.js": `export default { render() {} };`,
		});

		await buildFixture(fixture, {}, "@fixture/app");
		const manifest = await readManifest(fixture.outDir);

		expect(manifest.modules).toContain(manifest.app);
		expect((await stat(join(fixture.outDir, manifest.app))).isFile()).toBe(true);
	});
...
async function buildFixture(
	...
	await build({
		...
		root: fixture.root,
		logLevel: "silent",
		plugins: [anywidgetBundle({ app, outDir: fixture.outDir, ...overrides })],
	});
}

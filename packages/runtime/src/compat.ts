...
function defineRuntimeCompatibilityBuiltins<Name extends string>(
	builtins: Record<Name, RuntimeCompatibilityBuiltin>,
): Record<Name, RuntimeCompatibilityBuiltin> {
	return builtins;
}
...
// Widen factory results so declarations do not capture Notebook Kit's private
// generator types while preserving the builtin names as a literal union.
const RUNTIME_COMPAT_BUILTINS = defineRuntimeCompatibilityBuiltins({
	Generators: {

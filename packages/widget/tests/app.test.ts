...
describe("widget graph and notebook readback", () => {
	test("renders one Notebook runtime and publishes graph and cell readback on the parent model", async () => {
		...
		widget.render(renderProps(model, el, controller.signal, host));
	...
	test("marks a multi-output cell and notebook rendered after every output settles", async () => {
		...
		try {
			...
			expect(cellRecord(model, 0)).toBeUndefined();
			expect(model.get("_has_rendered")).toBe(false);

			resolveY(2);
			expect(await waitFor(() => (variableValue(model, "y") === 2 ? 2 : undefined), 1500)).toBe(2);
			expect(cellRecord(model, 0)).toMatchObject({ rendered: true, names: ["x", "y"] });
	...
	test("invalidates stale readback and graph before publishing a changed notebook", async () => {
		...
		model.set("_spec", { cells: [{ id: 1, mode: "ojs", value: "total = 7" }] });
		model.set("_cell_keys", ["total"]);

		expect(await waitFor(() => (variableValue(model, "total") === 7 ? 7 : undefined))).toBe(7);
		expect(variableValue(model, "answer")).toBeUndefined();
		expect((await waitFor(() => graphValue(model))).cells[0]?.key).toBe("total");
	...
	test("publishes named display values and runtime errors under the cell key", async () => {
		...
		expect(await waitFor(() => (variableValue(model, "readout") === 43 ? 43 : undefined))).toBe(43);
		const error = await waitFor(() => variableValue(model, "broken") as Record<string, unknown> | undefined);
...
`,
		...
		expect(variableValue(model, "rows")).toEqual([{ x: 10 }, { x: 20 }]);
	...
	test("renders syntax failures as settled cell output", async () => {
		...
		expect(await waitFor(() => (model.get("_has_rendered") === true ? true : undefined))).toBe(true);
		expect(cellRecord(model, 0)).toMatchObject({ rendered: true, values: {} });
		controller.abort();
	});
});
